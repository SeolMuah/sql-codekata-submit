// LeetCode Content Script (SQL 전용)
(function() {
  'use strict';

  let isWaitingForResult = false; // 제출 버튼 클릭 후 결과 대기 중
  let hasProcessedResult = false; // 현재 제출에 대해 이미 처리함
  ...
  function detectResult() {
    ...
    if (text.includes('accepted') || text.includes('success')) {
      return true;
    } else if (text.includes('wrong') || text.includes('error') ||
               text.includes('fail') || text.includes('time limit')) {
      return false;
    }
  ...
  async function submitToGoogleForm(problemInfo, code, githubUrl = '') {
    // Google Form 드롭다운과 동일한 형식: "번호 - LeetCode번호. 제목"
  ...
  async function handleSubmission(isCorrect) {
    ...
    // 등록된 문제 정보 조회
    let problemInfo = findProblemByProblemId(problemId, 'leetcode');
    ...
    console.log('[SPARTA] LeetCode 제출 감지:', { problemId, isCorrect, title: problemInfo.title, registered: isRegistered });
  ...
  async function init() {
    ...
    console.log('[SPARTA] LeetCode 코드카타 활성화 -', settings.studentName);
    observeSubmitButton();
